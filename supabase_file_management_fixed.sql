-- Dosya Yönetimi Tablosu (İzin Sorunları Çözülmüş)
...
-- RLS tamamen devre dışı
ALTER TABLE saved_files DISABLE ROW LEVEL SECURITY;
...
-- Dosya indirme logları için tablo
...
-- RLS tamamen devre dışı
ALTER TABLE file_download_logs DISABLE ROW LEVEL SECURITY;
...
GRANT EXECUTE ON FUNCTION log_file_download TO authenticated;
GRANT EXECUTE ON FUNCTION get_user_file_stats TO authenticated;
GRANT EXECUTE ON FUNCTION get_institution_file_stats TO authenticated;
...
-- Dosya yönetimi için gerekli tüm tablolar oluşturuldu
-- İzin sorunları çözülmüş versiyon

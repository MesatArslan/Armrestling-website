...
interface DoubleElimination257_383Props extends DoubleEliminationProps {
  resetKey?: number;
}
...
const DoubleElimination257_383: React.FC<DoubleElimination257_383Props> = ({ players, resetKey, onMatchResult, onTournamentComplete, initialTab, fixtureId }) => {
  ...
  const loadTournamentState = () => {
    try {
      const playerIds = players.map(p => p.id).sort().join('-');
      const state = DoubleEliminationStorage.getDoubleEliminationState(257, playerIds, fixtureId);
      if (state) {
  ...
  const initializeTournament = () => {
    ...
    const totalSlots = 512; // 257-383 players need 512 slots for WB1
  ...
  React.useEffect(() => {
    console.log('DoubleElimination257_383 useEffect - players:', players.length);
    if (players.length >= 257 && players.length <= 383) {
  ...
  function getMatchRoundKey(match: Match): RoundKey {
    ...
    if (match.id.startsWith('wb8')) return 'WB8';
    ...
    if (match.id.startsWith('lb14')) return 'LB14';
  ...
  function createNextRound(): Match[] {
    ...
    switch (nextRoundKey) {
      ...
      case 'LB2': {
        ...
        if (lb1Winners.length !== 128) return [];
      ...
      case 'LB3': {
        ...
        if (lb3Players.length !== 128) return [];
      ...
      case 'LB4': {
        ...
        if (lb3Winners.length !== 64) return [];
      ...
      case 'LB5': {
        ...
        if (lb5Players.length !== 64) return [];
      ...
      case 'LB6': {
        ...
        if (lb5Winners.length !== 32) return [];
      ...
      case 'LB7': {
        ...
        if (lb7Players.length !== 32) return [];
      ...
      case 'LB8': {
        ...
        if (lb7Winners.length !== 16) return [];
      ...
      case 'LB9': {
        ...
        if (lb9Players.length !== 16) return [];
      ...
      case 'LB11': {
        ...
        if (lb11Players.length !== 8) return [];
      ...
      case 'LB12': {
        ...
        if (lb11Winners.length !== 4) return [];
      ...
      case 'LB13': {
        ...
        const lb13Players = [...lb12Winners, ...wb8Losers];
        if (lb13Players.length !== 4) return [];
        const lb13Matches: Match[] = [];
        for (let i = 0; i < lb13Players.length; i += 2) {
          if (i + 1 < lb13Players.length) {
            lb13Matches.push({
              id: `lb13_${Math.floor(i/2) + 1}`,
              player1Id: lb13Players[i],
              player2Id: lb13Players[i + 1],
              bracket: 'loser',
              round: 13,
              matchNumber: Math.floor(i/2) + 1,
              isBye: false,
      ...
      case 'LB14': {
        ...
        if (lb13Winners.length !== 2) return [];
        const lb14Matches: Match[] = [];
        for (let i = 0; i < lb13Winners.length; i += 2) {
          if (i + 1 < lb13Winners.length) {
            lb14Matches.push({
              id: `lb14_${Math.floor(i/2) + 1}`,
              player1Id: lb13Winners[i],
              player2Id: lb13Winners[i + 1],
              bracket: 'loser',
              round: 14,
              matchNumber: Math.floor(i/2) + 1,
              isBye: false,
      ...
      case '7-8': {
        ...
        if (lb12Losers.length !== 2) {
          console.log('7-8: Yeterli LB12 kaybeden yok, 2 bekleniyor ama', lb12Losers.length, 'var');
          return [];
        }
        return [{
          id: 'seventh_eighth',
      ...
      case 'LBFinal': {
        ...
        if (yariFinalLoser.length !== 1 || lb14Winners.length !== 1) return [];
        return [{
          id: 'lbfinal',
          player1Id: lb14Winners[0],
      ...
      case '5-6': {
        ...
        if (wb8Losers.length !== 2 || lb12Losers.length !== 2) return [];
        return [{
          id: 'fifth_sixth',
          player1Id: wb8Losers[0],
          player2Id: lb12Losers[1],
          bracket: 'loser',
          round: 17,
          matchNumber: 1,
  ...
  const calculateRankings = (matchList: Match[]): Ranking => {
    ...
    const lb14Match = matchList.find(m => m.id === 'lb14_1');
    if (lb14Match?.winnerId) {
      rankings.fourth = lb14Match.winnerId === lb14Match.player1Id ? lb14Match.player2Id : lb14Match.player1Id;
    }
    ...
    return (
      <div className="text-center p-8">
        ...
        <p className="text-gray-600">
          Bu turnuva formatı 257-383 oyuncu arası için tasarlanmıştır. 
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="mb-6 flex justify-between items-center">
        <h2 className="text-2xl font-bold text-gray-800">
          Double Elimination Tournament (257-383 oyuncu)
        </h2>
        <button
...
export default DoubleElimination257_383; 
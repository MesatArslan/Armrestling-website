...
const DoubleElimination129_191: React.FC<DoubleElimination129_191Props> = ({ players, resetKey, onMatchResult, onTournamentComplete, onUpdateOpponents, onRemoveOpponents, onClearAllOpponents, fixtureId }) => {
  ...
  // Keep refs in sync with latest states for interval-based auto selection
        ...
        for (let i = 0; i < wb7Winners.length; i += 2) {
          if (i + 1 < wb7Winners.length) {
            yariFinalMatches.push({
              ...
              matchNumber: Math.floor(i/2) + 1,
              isBye: false,
              description: RoundDescriptionUtils.createMatchDescription('WB_SemiFinal', Math.floor(i/2) + 1)
            });
          }
        }
  ...
  const undoLastMatch = () => {
    ...
    if (lastId === 'grandfinal') {
      ...
      newCurrentRoundKey = '5-6';
    } else if (lastId === 'lb12_1') {
      clearWinner('lb12_1');
      delete updatedRankings.fourth;
      newCurrentRoundKey = 'LB12';
    } else {
  ...
  const handleMatchResult = (matchId: string, winnerId: string) => {
    ...
    if (grandFinalMatch?.winnerId) {
      ...
      // Update interim rankings every match
      updatedRankings = calculateRankings(updatedMatches);
    }

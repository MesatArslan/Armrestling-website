...
const Players = () => {
  ...
  const handleClearAllData = () => {
    if (window.confirm('Are you sure you want to clear all player data? This action cannot be undone.')) {
  ...
  const handleImportJSON = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        alert('Oyuncular başarıyla eklendi ve güncellendi!');
      } catch (err: any) {
      ...
      <div className="w-full max-w-7xl px-2 sm:px-6 lg:px-8">
        <div className="backdrop-blur-md bg-white/80 rounded-2xl border border-gray-200 shadow-2xl p-2 sm:p-6 transition-all duration-300">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 p-6 border-b border-gray-200">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 tracking-tight drop-shadow-sm">Players</h1>
              <p className="text-base text-gray-500 mt-1">Total Players: {players.length}</p>
            </div>
      ...
      {isAddColumnModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="backdrop-blur-lg bg-white/90 p-8 rounded-2xl w-96 shadow-2xl border border-gray-200">
            <h2 className="text-2xl font-bold mb-6 text-gray-900">Add New Column</h2>
            <input
              type="text"
              value={newColumnName}

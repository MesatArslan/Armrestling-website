-- Kurum Bazında Dosya Yönetimi Sistemi
-- Tüm kullanıcılar aynı kurumun storage limitini paylaşır
...
CREATE OR REPLACE FUNCTION check_institution_file_size_limit()
RETURNS TRIGGER AS $$
DECLARE
...
DROP TRIGGER IF EXISTS check_file_size_limit_trigger ON saved_files;
...
DROP TRIGGER IF EXISTS update_saved_files_updated_at ON saved_files;
...
CREATE OR REPLACE FUNCTION search_institution_files(
